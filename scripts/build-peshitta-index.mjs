...
const sourcePath = new URL('../public/biblia-peshitta-approved.pdf', import.meta.url)
...
const BOOKS = [
  ...
  ['1 SAMUEL', ['1° SAMUEL', '1ª SAMUEL', 'PRIMER LIBRO DE SAMUEL']], ['2 SAMUEL', ['2° SAMUEL', '2ª SAMUEL', 'SEGUNDO LIBRO DE SAMUEL']], ['1 REYES', ['1° REYES', '1ª REYES', 'PRIMER LIBRO DE REYES']], ['2 REYES', ['2° REYES', '2ª REYES', 'SEGUNDO LIBRO DE REYES']],
...
const verseStart = (line) => line.match(/^\s*(\d{1,3})\s+(.+?)\s*$/)
const chapterHeading = (index) => /^\s*(\d{1,3})\s*$/.exec(lines[index])
...
function findBookStart(book, from) {
  ...
  for (let i = from; i < lines.length; i += 1) {
    ...
    if (needles.some((needle) => line === needle || line.includes(needle))) return i
    if (book[0] === 'HECHOS' && fold(lines[i]) === 'LOS' && fold(lines[i + 1] || '') === 'HECHOS') return i
  }
  return -1
}
...
for (let b = 0; b < starts.length; b += 1) {
  ...
  for (let i = start; i < end; i += 1) {
    ...
    if (chapterMatch && Number(chapterMatch[1]) > chapter && fold(lines[i + 1] || '') && !verseStart(lines[i + 1] || '')) {
      chapter = Number(chapterMatch[1])
      current = null
      continue
    }
    const match = verseStart(lines[i])
    if (match && chapter >= 1 && !isPageNumber(lines[i])) {
      const verse = Number(match[1])
      if (verse >= 1 && verse <= 200) {
        current = { libro: book, capitulo: chapter, versiculo: verse, parts: [match[2]] }
        rows.push(current)
        continue
      }
    }
