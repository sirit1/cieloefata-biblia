 ...
 * passage. Version (RVR1960/NVI/DHH/TLA) only chooses the STRICT quote of
 * each ref — never whether refs exist, never a substitute version's wording.
 * If that version has no verse text, refs still return with quoteMissing.
 * Empty only when TSK has no row for that coordinate. No per-verse special cases.
 *
...
/**
 * Quote ONLY the consulted version. Empty quote + quoteMissing if that
 * version has no verse text. Never paste RVR as DHH/TLA/NVI.
 */
async function citarEnVersion(refObj, versionMeta) {
  try {
    const got = await obtenerTextoVersion(refObj, versionMeta.bolls);
    const texto = textoDeVersos(got);
    if (texto && !esCitaInglesa(texto)) {
      return { texto, quoteMissing: false };
    }
  } catch {
    /* pack / remote miss */
  }
  return { texto: '', quoteMissing: true };
}
...
export async function obtenerTsk(input = {}) {
  ...
  const out = await Promise.all(
    pending.map(async (row) => {
      const quoted = await citarEnVersion(row.refObj, versionMeta);
      return {
        ref: row.cita,
        texto: quoted.texto,
        quoteMissing: quoted.quoteMissing,
        nota: quoted.quoteMissing
          ? `TSK / OpenBible · sin cita de ${versionMeta.etiqueta} (pack o Bolls vacío)`
          : 'TSK / OpenBible',
      };
    }),
  );

  const quotePackMissing = out.length > 0 && out.every((row) => row.quoteMissing);
  ...
  return {
    ...
    data: {
      ...
      fuente: 'tsk-open-cross-ref',
      quotePackMissing,
      quotePackNote: quotePackMissing
        ? `${versionMeta.etiqueta} no tiene texto para estas citas. Las referencias TSK sí existen.`
        : '',
      referencias: out,
    },
  };
