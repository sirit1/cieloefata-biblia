...
import { ensureCommentarySpanish, lookupStoredVerse } from './comentario-es.js';
...
export async function obtenerComentarioCorpus({ passage, author } = {}) {
  ...
  try {
    ...
    if (helloaoId) {
      ...
      if (note?.text) {
        const text = helloaoId === 'john-calvin'
          ...
          : note.text;
        return ensureCommentarySpanish(hitPayload(authorName, etiqueta, authorId, `corpus:${helloaoId}`, text));
      }
      if (authorId === 'adam-clarke') {
        const acc = await loadClarkeAccNotes(ref.slug, ref.capitulo);
        const accNote = pickNote(acc, ref.verse);
        if (accNote?.text) {
          return ensureCommentarySpanish(
    ...
    if (got?.text) {
      return ensureCommentarySpanish(hitPayload(authorName, etiqueta, authorId, got.source, got.text));
    }
