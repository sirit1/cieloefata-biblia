...
export function resolveAuthorId(raw) {
  ...
  if (s.includes('jamieson') || s.includes('jfb') || s.includes('fausset') || s.includes('brown')) {
...
function parseSpeNotes(html, bookLabel, chapter) {
  ...
  const heading = new RegExp(
    String.raw`(?:Verses?\s+(\d{1,3})\s*[-–—]\s*(\d{1,3})|${book}\s+${chapter}:(\d{1,3})(?:-(\d{1,3}))?)`,
    'gi',
  );
  const hits = [];
  let m;
  while ((m = heading.exec(plain))) {
    ...
    const end = Number(m[2] || m[4] || start);
...
async function loadSpeNotes(slug, chapter, bookLabel) {
  ...
  return cached(key, async () => {
    const got = await fetchText(`${SPE_BASE}/${slug}-${chapter}.php`);
    if (!got.ok) return [];
    return parseSpeNotes(got.text, bookLabel, chapter);
  });
}
...
async function fetchWesley(ref) {
  ...
  const note = pickNote(htmlNotes, ref.verse);
  if (note?.text) return { source: 'corpus:john-wesley', text: note.text };
  return null;
}
...
export async function obtenerComentarioCorpus({ passage, author } = {}) {
  ...
  try {
    ...
    if (helloaoId) {
      ...
      if (note?.text) {
        return hitPayload(authorName, etiqueta, authorId, `corpus:${helloaoId}`, note.text);
      }
    ...
    if (got?.text) return hitPayload(authorName, etiqueta, authorId, got.source, got.text);
    return missPayload(authorName, etiqueta, authorId);
  } catch (err) {
