...
function splitForMyMemory(text) {
  ...
  if (src.length <= MYMEMORY_MAX) return src ? [src] : [];
  const parts = [];
  let buf = '';
  ...
  for (const s of sentences) {
    if (s.length > MYMEMORY_MAX) {
      flush();
      for (let i = 0; i < s.length; i += MYMEMORY_MAX) parts.push(s.slice(i, i + MYMEMORY_MAX));
      continue;
    }
    const next = buf ? `${buf} ${s}` : s;
    if (next.length > MYMEMORY_MAX && buf) {
      flush();
      buf = s;
    } else {
      buf = next;
...
async function translateMyMemoryPiece(text) {
  const q = String(text || '').trim();
  if (!q) return '';
  if (q.length > MYMEMORY_MAX) throw new Error('mymemory piece too long');
  const url =
    'https://api.mymemory.translated.net/get?langpair=en|es&q=' + encodeURIComponent(q);
...
async function translateChunkMyMemory(text) {
  ...
  if (!pieces.length) throw new Error('mymemory empty');
  const out = [];
  for (const piece of pieces) {
    out.push(await translateMyMemoryPiece(piece));
  }
  const joined = out.join(' ').trim();
  if (!joined) throw new Error('mymemory empty');
  return joined;
}
...
async function translateOnce(english) {
  ...
  const enLen = String(english || '').trim().length;
  if (enLen >= 500 && joined.length < enLen * 0.4) {
