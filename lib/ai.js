...
function resolverModelos(perfil = 'profundo') {
  ...
  if (geminiKey) {
    ...
    return perfil === 'rapido'
      ? [google(MODELO_GEMINI_FLASH), google(MODELO_GEMINI_PRO)]
      : [google(MODELO_GEMINI_PRO), google(MODELO_GEMINI_FLASH)];
  }
  return perfil === 'rapido'
    ? [MODELO_GATEWAY_FLASH, MODELO_GATEWAY_PRO]
    : [MODELO_GATEWAY_PRO, MODELO_GATEWAY_FLASH];
}
...
export async function generarJSON(prompt, { reintentos = 1, maxOutputTokens = 1800, perfil = 'profundo' } = {}) {
  let ultimoError;
