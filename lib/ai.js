...
// AI_GATEWAY_API_KEY. NO se usa Google/Gemini bajo ninguna circunstancia
// (la API de Gemini está bloqueada para este proyecto) — el modelo es
// OpenAI, servido zero-config a través del AI Gateway ya contratado.
//
...
export function hayMotorIA() {
  if (String(process.env.AI_GATEWAY_API_KEY || '').trim()) return true;
  if (process.env.VERCEL && String(process.env.VERCEL_OIDC_TOKEN || '').trim()) return true;
  return false;
}

/** Texto libre vía AI Gateway (comentarios, TSK, lentes). */
export async function generarTexto(prompt, { maxOutputTokens = 2800 } = {}) {
  if (!hayMotorIA()) return '';
