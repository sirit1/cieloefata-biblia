...
export async function generarObjetoDual(prompt, { schema, maxOutputTokens = 8000 } = {}) {
  ...
  const geminiKey = process.env.GEMINI_API_KEY || process.env.GOOGLE_GENERATIVE_AI_API_KEY;
  const [proRes, flashRes] = geminiKey
    ? await Promise.allSettled([
        intentarObjeto(pro, prompt, schema, maxOutputTokens),
        intentarObjeto(flash, prompt, schema, maxOutputTokens),
      ])
    : [
        { status: 'rejected', reason: new Error('Modelo Pro no disponible en el gateway de preview.') },
        await intentarObjeto(flash, prompt, schema, maxOutputTokens).then(value => ({ status: 'fulfilled', value })).catch(reason => ({ status: 'rejected', reason })),
      ];
