...
export function buscarPeshitta(consulta) {
  ...
  if (reference) {
    const referenciasUnicas = [...new Set(resultados.map((row) => row.referencia))];
    // The legacy PDF extraction can assign several unrelated fragments to the
    // same reference. Never concatenate them or present them as one verse.
    if (referenciasUnicas.length !== 1 || resultados.length !== 1) {
      return { texto: '', versos: [], fuente: PDF_SOURCE };
    }
    const row = resultados[0];
    return { texto: `${row.referencia} ${row.texto}`, versos: [row], fuente: PDF_SOURCE };
  }
