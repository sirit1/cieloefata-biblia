...
export const VERSIONES = [
  ...
  { key: 'kjv', bolls: 'KJV', etiqueta: 'KJV' },
...
// v1.0: primero el léxico Strong local de dominio público; luego BDB/Thayer (también PD).
export const DICCIONARIO_STRONG = { bolls: 'BDBT', nombre: 'Concordancia Strong · dominio público' };
...
export async function fetchConTimeout(url, opts = {}, ms = 12000) {
...
function limpiarHtmlVerso(texto) {
  return String(texto || '')
    .replace(/<br\s*\/?>/gi, ' ')
    // Strong's markers from Bolls (e.g. word<S>1161</S>) must go before tag strip.
    .replace(/<S>\d+<\/S>/gi, '')
    ...
    .replace(/<[^>]+>/g, '')
    // Si quedó pegado el nº Strong (And1161 → And)
    .replace(/([A-Za-zÁÉÍÓÚÜÑáéíóúüñ'’]{2,})(\d{3,5})\b/g, '$1')
    .replace(/&nbsp;/g, ' ')
    .replace(/&amp;/g, '&')
    .replace(/&quot;/g, '"')
    .replace(/&#39;/g, "'")
...
function numeroVersoCrudo(v, fallbackIndex) {
  const n = Number(v?.verse ?? v?.n ?? v?.number ?? v?.versiculo ?? v?.verso ?? fallbackIndex);
...
export function extraerTexto(versos, versoInicio, versoFin) {
  ...
  if (filtrados.length === 1) return limpiarHtmlVerso(textoVersoCrudo(filtrados[0]));
  return filtrados
    .map((v, i) => `${numeroVersoCrudo(v, i + 1)} ${limpiarHtmlVerso(textoVersoCrudo(v))}`)
    .join(' ');
}
...
export function extraerVersos(versos, versoInicio, versoFin) {
  ...
  return filtrados
    .map((v, i) => ({
      n: numeroVersoCrudo(v, i + 1),
      texto: limpiarHtmlVerso(textoVersoCrudo(v)),
    }))
    .filter((v) => v.n > 0 && v.texto && !/^[\s.…·•-]*$/.test(v.texto));
}
...
// Caché en memoria para entradas léxicas: las definiciones Strong son inmutables.
const cacheStrong = new Map();
const TTL_STRONG_MS = 24 * 60 * 60 * 1000;
...
export async function obtenerDefinicionStrong(codigo) {
  ...
  if (!/^[GH]\d{1,5}$/.test(limpio)) return null;
  const cacheada = cacheStrong.get(limpio);
  if (cacheada && Date.now() - cacheada.t < TTL_STRONG_MS) return cacheada.data;
  const local = entradaStrongLocal(limpio);
  if (local?.definicion) {
    const resultado = {
      codigo: limpio,
      lexema: local.lemma || '',
      ...
      lemma: local.lemma || '',
      raiz: local.raiz || '',
      idioma: local.idioma || (limpio.startsWith('H') ? 'hebreo' : 'griego'),
  ...
  const d = data[0];
  const original = String(d.short_definition || (d.definition || '').replace(/<[^>]+>/g, ' ').split(/[.;]/)[0] || '').replace(/\s+/g, ' ').trim();
  const catalogada = glosaEspanol(original, limpio);
  const resultado = {
    codigo: limpio,
    lexema: d.lexeme || '',
    transliteracion: d.transliteration || '',
    pronunciacion: d.pronunciation || '',
    definicionCorta: original,
    definicion: original,
    fuente: 'Strong / Thayer / BDB · dominio público',
    definicionEs: catalogada,
    traduccionEstricta: catalogada,
    sinGlosaEs: !catalogada,
  };
  cacheStrong.set(limpio, { data: resultado, t: Date.now() });
  return resultado;
}
