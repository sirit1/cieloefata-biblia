 ...
 * Local pack first when it has text; else Bolls. Failed probes are not
 * cached as empty. Unhosted versions do not fake «no hay coincidencias»:
 * search a hosted version (RVR1960) and name it in an honest banner.
 * Timeouts/HTTP throw (502) — never «0 coincidencias».
 */
import { fetchConTimeout, resaltarCoincidencia, LIBROS, resolverVersion } from './biblia.js';
import { cargarPack } from './versiones.js';

const MAX_RESULTADOS = 40;
const FALLBACK_VERSION = 'rv1960';
 ...
 * errors return null and are not cached.
...
function payloadFor(q, meta, { resultados, fuente, total, indexable }) {
  return {
    termino: q,
    version: meta.key,
...
async function searchVersion(q, meta) {
  ...
  const resultados = mapBollsHits(data, q);
...
export async function obtenerConcordancia({ termino, version } = {}) {
  ...
  const primary = await searchVersion(q, meta);
  if (primary.resultados.length || primary.indexable) {
    return { success: true, data: primary };
  }

  if (meta.key === FALLBACK_VERSION) {
    return { success: true, data: primary };
  }

  const rvMeta = resolverVersion(FALLBACK_VERSION);
  const rv = await searchVersion(q, rvMeta);
  return {
    success: true,
    data: {
      termino: q,
      ...
      indexable: false,
      fallbackUsed: true,
      fallbackVersion: rv.version,
      fallbackEtiqueta: rv.etiqueta,
      banner: `${meta.etiqueta} no tiene índice. Los resultados son de ${rv.etiqueta}.`,
      total: rv.total,
      resultados: rv.resultados,
      fuente: rv.fuente,
    },
  };
}
