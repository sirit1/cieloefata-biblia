# Cobertura comentario ES (PR #16)

Preview: https://revelatio-git-cursor-comentarios-corpus-46d945-sirit1s-projects.vercel.app

Capa almacenada. No se traduce en cada request. Disclaimer: «Traducción automática del original inglés (dominio público). No es la edición de CLIE.»

## API preview (comprobado)

| Pasaje | Autor | found | translated | text/textEs | textEn |
| --- | --- | --- | --- | --- | --- |
| Mateo 9:12 | Henry | true | true | ES | EN |
| Mateo 9:12 | Spurgeon | true | true | ES | EN |
| Mateo 9:12 | Clarke | true | true | ES | EN |
| Lucas 15:18 | Henry / Spurgeon | true | true | ES | EN |
| Juan 14:6 | Henry | true | true | ES | EN |
| Filemón 1:5 | Henry | true | true | ES | EN |
| Apocalipsis 1:1 | Henry | true | true | ES | EN |
| Mateo 16:2 | Spurgeon | false | — | No hay nota | — |
| Marcos 1:1 | Spurgeon | false | — | No hay nota (SPE no tiene hit) | — |

## Archivos persistidos (blobs con `es`)

| Libro | Henry | Spurgeon | Clarke | Wesley | Calvino | JFB | Gill |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Mateo | sí (caps. 1–18 helloao; 19–28 helloao 404) | sí | sí (ACC PD) | sí | sí | sí | sí |
| Marcos | sí | sí | sí | pendiente | — | — | — |
| Lucas | sí | sí | sí | sí | sí | sí | sí |
| Juan | sí | sí | sí | sí | sí | sí | en curso |
| Hechos | sí | sí | sí | pendiente | — | — | — |
| Romanos | sí | sí | sí | pendiente | — | — | — |
| 1 Corintios | sí | sí | sí | pendiente | — | — | — |
| 2 Corintios | sí | sí | sí | pendiente | — | — | — |
| Gálatas | sí | sí | sí | pendiente | — | — | — |
| Efesios | sí | sí | sí | pendiente | — | — | — |
| Filipenses | sí | sí | sí | pendiente | — | — | — |
| Colosenses … Apocalipsis | sí | sí | en curso | pendiente | — | — | — |
| Filemón | sí | 1 hit SPE | sí | sí | sí | sí | pendiente |

Henry NT: **completo** salvo el hueco honesto de Mateo 19–28 (helloao 404).
Spurgeon NT: **completo** donde SPE tiene inglés. 2 Juan y 3 Juan: 0 hits SPE (miss honesto). Filemón: 1 hit.
JFB: **completo** en los 4 libros de prioridad.

## Notas

- Helloao Henry omite Mateo 19–28: miss honesto si no hay inglés.
- Helloao Clarke omite todo Mateo: se usa ACC PD (truthaccordingtoscripture / StudyLight).
- Spurgeon Filemón: un solo hit SPE; el resto es «No hay nota».
- Calvino / JFB / Gill: solo Mateo, Juan, Lucas, Filemón en esta corrida (volumen).
- Treasury of David: no ingerido (sin JSON limpio).
- Sin Guzik. Sin «certificada». Sin traducción en vivo.
